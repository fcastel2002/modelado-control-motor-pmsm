...
\vspace{1mm}
{\small Relación cinemática: $\frac{d\theta_l(t)}{dt}=\omega_l(t)$.}
\end{frame}
...
\begin{itemize}
\item $T_m(t)$: torque electromagnético entregado por las corrientes del estator vía la interacción con el flujo de los imanes.
...
\begin{equation*}
T_d(t) = \tfrac{J_l}{r^2}\,\frac{d\omega_m(t)}{dt} + \tfrac{b_l}{r^2}\,\omega_m(t) + \tfrac{1}{r}\!\left[g\,k_l\,\sin\!\big(\tfrac{\theta_m(t)}{r}\big) + T_{ld}(t)\right]
\end{equation*}

\textbf{Paso 3.} Reemplazando $T_d$ en la ecuación del rotor ($J_m\frac{d\omega_m}{dt}=T_m-b_m\omega_m-T_d$) y agrupando términos en $\frac{d\omega_m}{dt}$ y $\omega_m$ se obtiene la ecuación equivalente compactada (siguiente slide).
\end{frame}
...
\begin{equation*}
\boxed{\;\;
J_{eq}\,\frac{d\omega_m(t)}{dt} = T_m(t) - b_{eq}\,\omega_m(t) - \tfrac{1}{r}\!\left[g\,k_l\,\sin\!\big(\tfrac{\theta_m(t)}{r}\big) + T_{ld}(t)\right]
\;\;}
\end{equation*}
...
\begin{equation*}
\begin{cases}
\frac{d\theta_m(t)}{dt} = \omega_m(t) \\[4pt]
\frac{d\omega_m(t)}{dt} = -\dfrac{b_{eq}}{J_{eq}}\,\omega_m(t) -\dfrac{g\,k_l}{J_{eq}\,r}\,\sin\!\big(\tfrac{\theta_m(t)}{r}\big) +\dfrac{1}{J_{eq}}\,T_m(t) -\dfrac{1}{J_{eq}\,r}\,T_{ld}(t) \\[6pt]
y(t) = \theta_m(t)
\end{cases}
\end{equation*}

\vspace{2mm}
\textbf{Forma matricial} --- el término $\sin(\cdot)$ impide escribir $\frac{d\bm{x}}{dt}=\bm{A}\bm{x}+\bm{B}\bm{u}$ estándar; el sistema es \textbf{no lineal}:
\begin{equation*}
\begin{bmatrix}\frac{d\theta_m(t)}{dt} \\ \frac{d\omega_m(t)}{dt}\end{bmatrix} =
...
\left\{
\begin{aligned}
\frac{d\theta_m(t)}{dt} &= \omega_m(t) \\[1pt]
\frac{d\omega_m(t)}{dt} &= -\tfrac{b_{eq}}{J_{eq}}\omega_m(t) - \tfrac{g\,k_l}{J_{eq}\,r}\sin\!\big(\tfrac{\theta_m(t)}{r}\big) + \tfrac{1}{J_{eq}}\,T_m(t) - \tfrac{1}{J_{eq}\,r}\,T_{ld}(t) \\[1pt]
\frac{di_{qs}^{r}(t)}{dt} &= \tfrac{1}{L_q}\!\left[\,v_{qs}^{r}(t) - R_s\!\big(T_s^\circ(t)\big)\, i_{qs}^{r}(t) - \big(\lambda'^r_m + L_d\,i_{ds}^{r}(t)\big)\,\omega_r(t)\,\right] \\[1pt]
\frac{di_{ds}^{r}(t)}{dt} &= \tfrac{1}{L_d}\!\left[\,v_{ds}^{r}(t) - R_s\!\big(T_s^\circ(t)\big)\, i_{ds}^{r}(t) + L_q\,i_{qs}^{r}(t)\,\omega_r(t)\,\right] \\[1pt]
\frac{di_{0s}(t)}{dt} &= \tfrac{1}{L_{ls}}\!\left[\,v_{0s}(t) - R_s\!\big(T_s^\circ(t)\big)\, i_{0s}(t)\,\right] \\[1pt]
\frac{dT_s^\circ(t)}{dt} &= \tfrac{1}{C_{ts}}\!\left[\,\tfrac{3}{2}R_s\!\big(T_s^\circ(t)\big)\big(i_{qs}^{r\,2}(t)+i_{ds}^{r\,2}(t)+2\,i_{0s}^{2}(t)\big) - \tfrac{T_s^\circ(t) - T_{amb}^\circ(t)}{R_{ts\text{-}amb}}\,\right]
\end{aligned}\right.
\end{equation*}
\normalsize
...
\frac{d\omega_m}{dt} &= -\tfrac{b_{eq}}{J_{eq}}\omega_m - \tfrac{g\,k_l}{J_{eq}\,r}\sin\!\big(\tfrac{\theta_m}{r}\big) + \tfrac{1}{J_{eq}}\,T_m(i_{qs}^{r},\,i_{ds}^{r}) - \tfrac{1}{J_{eq}\,r}\,T_{ld} \\
\frac{di_{qs}^{r}}{dt} &= \tfrac{1}{L_q}\!\big[\,v_{qs}^{r} - R_s\!\big(T_s^\circ\big)\, i_{qs}^{r} - (\lambda'^r_m + L_d\,i_{ds}^{r})\,\omega_r\,\big] \\
\frac{di_{ds}^{r}}{dt} &= \tfrac{1}{L_d}\!\big[\,v_{ds}^{r} - R_s\!\big(T_s^\circ\big)\, i_{ds}^{r} + L_q\,i_{qs}^{r}\,\omega_r\,\big] \\
\frac{dT_s^\circ}{dt} &= \tfrac{1}{C_{ts}}\!\big[\,\tfrac{3}{2}R_s\!\big(T_s^\circ\big)(i_{qs}^{r\,2}+i_{ds}^{r\,2}) - \tfrac{T_s^\circ - T_{amb}^\circ}{R_{ts\text{-}amb}}\,\big]
\end{aligned}\right.
\end{equation*}
\normalsize
...
\subsection{Implementación en Entorno de Simulación}

% -------------------------------------------------------------------
%  SLIDE 32 - Diagrama Simulink completo (apertura de validación)
% -------------------------------------------------------------------
\figslide{Modelo del accionamiento implementado en Simulink}{%
Implementación del modelo no lineal completo del accionamiento en MATLAB/Simulink. La planta recibe las tensiones de fase $v_{abcs}$ y el torque de carga $T_{ld}$, y entrega la posición y la velocidad ($\theta_m,\omega_m$), las corrientes de fase, la temperatura del estator $T_s^\circ$ y el torque. Los tres subsistemas (electromagnético, mecánico y térmico) interactúan simultáneamente; las transformaciones de Park directa e inversa vinculan el marco $abc$ con el marco rotórico $qd0$.%
}{simulink_diagrama_completo.png}{Diagrama de bloques del modelo del accionamiento implementado en Simulink, con la interconexión de los subsistemas electromagnético, mecánico y térmico.}
...
\begin{equation*}
\frac{d\bm{x}}{dt}=\bm{f}(\bm{x},\bm{u})
\end{equation*}

\textbf{Taylor de primer orden en $(\bm{x}_o,\bm{u}_o)$:}
\begin{equation*}
\Delta\frac{d\bm{x}(t)}{dt}\approx
...
\begin{itemize}
  \item \textbf{Cinemática:} $\frac{d\theta_{m,o}(t)}{dt}=\omega_{m,o}(t)$ con $\omega_{m,o}$ constante.
...
\begin{equation*}
\Delta\frac{d\bm{x}(t)}{dt}=\bm{A}_o(\rho)\,\Delta\bm{x}(t)+\bm{B}_o(\rho)\,\Delta\bm{u}(t)
\end{equation*}
...
% -------------------------------------------------------------------
\begin{frame}{Segundo método de linealización: realimentación de no linealidades}
...
\begin{equation*}
\frac{di_{ds}^{r}(t)}{dt}=\frac{1}{L_d}\Big[\,v_{ds}^{r}(t)-R_{s0}i_{ds}^{r}(t)+L_q\,i_{qs}^{r}(t)\,\omega_r(t)\Big]
\end{equation*}
...
\left\{
\begin{aligned}
\frac{d\theta_m(t)}{dt} &= \omega_m(t) \\[2pt]
\frac{d\omega_m(t)}{dt} &= \frac{1}{J_{eq}}\!\left[\frac{3}{2}P_p\lambda'^r_m\,i_{qs}^r(t)-b_{eq}\omega_m(t)-\frac{1}{r}T_l(t)\right] \\[4pt]
\frac{di_{qs}^r(t)}{dt} &= \frac{1}{L_q}\!\left[-P_p\lambda'^r_m\,\omega_m(t)-R_{s0}i_{qs}^r(t)+v_{qs}(t)\right]
\end{aligned}
\right.
\end{equation*}
\normalsize
...
\begin{equation*}
\frac{di_{ds}^{r}(t)}{dt}= -\frac{R_{s0}}{L_d}\,i_{ds}^{r}(t)
\end{equation*}
...
\begin{equation*}
v_{qs}^{r}(t)=L_q\frac{di_{qs}^{r}(t)}{dt}+R_s i_{qs}^{r}(t)+\lambda'^r_m\omega_r(t)+
...
\frac{d\theta_m(t)}{dt} &= \omega_m(t) \\[2pt]
\frac{d\omega_m(t)}{dt} &= \frac{1}{J_{eq}}\!\left[\frac{3}{2}P_p\lambda'^r_m\,i_{qs}^r(t)-b_{eq}\omega_m(t)-\frac{1}{r}T_l(t)\right] \\[4pt]
\frac{di_{qs}^r(t)}{dt} &= \frac{1}{L_q}\!\left[-P_p\lambda'^r_m\,\omega_m(t)-R_{s0}i_{qs}^r(t)+v_{qs}(t)\right] \\[4pt]
\frac{di_{ds}^{r}(t)}{dt} &= -\frac{R_{s0}}{L_d}\,i_{ds}^{r}(t)
\end{aligned}
\right.
\end{equation*}
...
}{polos_ceros_LA.png}{Mapa de polos y ceros a lazo abierto para ambas funciones de transferencia del modelo LTI aumentado (polos: cuadrados sólidos; ceros: círculos). Se indica con un rombo rojo el polo cancelado de $i_{ds}^r$.}
...
$\theta_m$ crece como rampa durante los pulsos de tensión, consecuencia directa del integrador puro $\frac{d\theta_m}{dt} = \omega_m$. Alcanza un máximo $\approx 242$~rad en $t \approx 0{,}7$~s, se mantiene durante $v_{qs}^{r*} = 0$ y retorna hacia cero durante el pulso negativo.%
...
Error acotado por debajo de $0{,}1$~rad ($<0{,}05\%$ de la excursión de $242$~rad). No se anula en régimen permanente: exhibe una deriva lenta acumulativa, porque $\theta_m$ es la integral de $\omega_m$ y el integrador acumula las pequeñas discrepancias instantáneas de velocidad. Offset residual $\approx 0{,}085$~rad al final del ensayo. Es comportamiento inherente al integrador puro de la planta, no error de modelado.%
}{sim_theta_m_error.png}{Error de seguimiento en $\theta_m(t)$: diferencia $\theta_m^{NL}-\theta_m^{LTI}$. \textbf{Error relativo $< 0{,}05\%$ de la excursión de 242~rad.}}
...
Concordancia NL-LTI excelente: error prácticamente nulo en régimen ($<0{,}5$~rad/s) y picos $\leq 3$~rad/s únicamente en las conmutaciones, que se extinguen en milisegundos.%
}{sim_omega_m_error.png}{Error de seguimiento en $\omega_m(t)$: diferencia $\omega_m^{NL}-\omega_m^{LTI}$. \textbf{Error nulo en régimen ($<0{,}5$~rad/s); picos $\leq 3$~rad/s en conmutaciones.}}

% -------------------------------------------------------------------
%  SLIDE 83 — Corriente i_qs
% -------------------------------------------------------------------
\figslide{Corriente de eje $q$, $i_{qs}^r(t)$}{%
Pico de torque transitorio $\approx \pm 10{,}4$~A (no $v_{qs}^{r*}/R_{s0}$) que decae hasta $\approx 0{,}1$~A en régimen, a medida que la FCEM $\lambda'^r_m P_p \omega_m$ equilibra la tensión aplicada. El pico es el valor relevante frente a $I_{s,max} = 2$~A rms (límite ampliamente superado en este ensayo LA). Como $i_{ds}^{r} \equiv 0$, $T_m = k_t\,i_{qs}^r$ con $k_t = \tfrac{3}{2}P_p\lambda'^r_m$: la forma de onda de $T_m(t)$ es proporcional a $i_{qs}^r(t)$.%
...
Los modelos se solapan casi perfectamente: error nulo en régimen y picos $\leq 0{,}1$~A únicamente en las conmutaciones, confirmando que las leyes de control NL linealizan efectivamente la dinámica de la planta.%
}{sim_iqs_error.png}{Error de seguimiento en $i_{qs}^r(t)$: diferencia $i_{qs}^{r,NL}-i_{qs}^{r,LTI}$. \textbf{Error nulo en régimen; picos $\leq 0{,}1$~A en conmutaciones.}}
...
% -------------------------------------------------------------------
\figslide{Tensión $v_{ds}^{r}(t)$ de la ley de control mínima}{%
La ley de control mínima impone internamente $v_{ds}^{r}=-L_q\,i_{qs}^{r}\,\omega_r$, proporcional al producto $i_{qs}^{r}\cdot\omega_m$. Esta tensión es no nula solo cuando ambas variables lo son simultáneamente, y su magnitud crece con el nivel de operación del sistema.%
}{sim_v_ds.png}{Tensión $v_{ds}^{r}$ del modelo NL ($i_{ds}^r(0) = 0$~A).}
...
% -------------------------------------------------------------------
\figslide{Curva paramétrica $T_m(\omega_m)$: cuadrantes de operación}{%
...
{\small\color{uncGray}Se agrega una consigna de tensión $v_{ds}^{r*}$ sumada a la ley de control mínima, con $i_{ds}^{r}(0)=0$~A para aislar el efecto de la tensión inyectada.}\par
\vspace{2mm}

\textbf{Consigna agregada a la ley mínima:}
\begin{equation*}
v_{ds}^{r}(t) = \underbrace{-L_q\,i_{qs}^{r}\,\omega_r}_{\text{ley mínima}} + v_{ds}^{r*}(t)
\end{equation*}
...
\vspace{2mm}
{\small \textbf{Amplitud:} $V_{ds}^{r*} = V_{qs,nom}/10 = 1{,}9596$~V, escalón aplicado a partir de $t=0{,}5$~s. $i_{ds}^{r}(0)=0$~A para aislar el efecto de la tensión.}
\end{frame}
...
A partir de $t=0{,}5$~s, $i_{ds}^{r}$ crece exponencialmente hasta $\pm 1{,}85$~A según el signo de la consigna; el caso base ($v_{ds}^{r*}=0$) permanece en cero. Comparación de los tres escenarios sobre la misma corriente de eje $d$.%
}{sim_ff_ids.png}{Efecto del field forcing ($v_{ds}^{r*} = +1{,}96$~V) y field weakening ($v_{ds}^{r*} = -1{,}96$~V) sobre $i_{ds}^r(t)$, comparado con el caso base ($v_{ds}^{r*} = 0$).}
...
v_{0s}^{*}(t) &= L_{ls}\,\frac{d i_{0s}(t)}{dt}
\end{align*}
...
Evolución temporal del par neto $T_{\text{neto}}$ [N$\cdot$m]. Se contrasta el comportamiento con resistencia estatórica constante (RS Cte., línea naranja) y con resistencia estatórica variable (RS Variable, línea azul).%
}{comp_torque_rs.png}{Evolución temporal del par neto $T_{\text{neto}}$~[N$\cdot$m]: comparación con $R_s$ constante (naranja) y $R_s$ variable (azul). Déficit de torque $\sim 15\%$.}
...
Velocidad angular mecánica $\omega_m(t)$ [rad/s] con resistencia estatórica constante (RS Cte., línea naranja) y con resistencia estatórica variable (RS Variable, línea azul).%
}{comp_velocidad_rs.png}{Velocidad angular mecánica $\omega_m(t)$~[rad/s]: comparación con $R_s$ constante (naranja) y $R_s$ variable (azul). El déficit de torque ($\sim 15\%$) se integra en un error de posición $\theta_m \sim 4\%$.}
...
\item $J_{eq}\,\frac{d^2\theta_m(t)}{dt^2}=T_m^{*\prime}(t)-T_{ld}(t)/r$.
\end{itemize}
...
Polos en el plano $s$ (cuadrados sólidos, diferenciados por color): planta a lazo abierto (azul), lazos de corriente (verde, $-5000$~rad/s) y polos de lazo cerrado del PID (rojo, $-800$ y $-600\pm j\,529$). Izquierda: vista completa mostrando la separación de escalas. Derecha: zoom planta vs.\ PID.%
}{mapa_polos_5_2_2.png}{Polos en el plano $s$ (cuadrados sólidos por color): planta a lazo abierto (azul), lazos de corriente (verde, $-5000$) y polos de lazo cerrado del PID (rojo, $-800$ y $-600 \pm j\,529$).}
...
Trayectoria de los polos de lazo cerrado al barrer $m_l$ de $0$ a $1{,}5$~kg con ganancias fijas nominales. \emph{Colorbar} en $m_l$. El nominal ($m_l=0$) se marca con rombo sólido y el peor caso ($m_l=1{,}5$~kg) con cuadrado sólido.%
}{migracion_polos_pid.png}{Migración de los polos de lazo cerrado del PID al variar $m_l$ de $0$ a $1{,}5$~kg con ganancias fijas. Nominal ($m_l=0$): rombo sólido; peor caso ($m_l=1{,}5$): cuadrado sólido.}
...
\[
\frac{d\hat{\bm{x}}}{dt}=\bm{A}\hat{\bm{x}}+\bm{B}u+\bm{K}_e\big(y-\bm{C}\hat{\bm{x}}\big)
\]
}
...
\begin{align*}
\frac{d\theta_m(t)}{dt} &= \omega_m(t) \\[6pt]
\frac{d\omega_m(t)}{dt} &= \frac{1}{J_{eq}}\,\Big(\,T_m^{*\prime}(t) - \frac{T_{ld}(t)}{r}\,\Big)
\end{align*}
...
{\small\color{uncGray}Espacio de estados estándar $\frac{d\bm{x}}{dt}=\bm{A}\,\bm{x}+\bm{B}\,u$, $y=\bm{C}\,\bm{x}$. La salida medida es $\theta_m$, la entrada el torque mecánico de referencia.}\par
\vspace{3mm}
...
\begin{align*}
\frac{d\bar{\bm{x}}}{dt} &= \bm{A}\,\bar{\bm{x}} + \begin{bmatrix} 0 \\ \tfrac{1}{J_{eq}} \end{bmatrix} u(t) + \bm{K}_e\,\big(\,y - \bm{C}\,\bar{\bm{x}}\,\big) \\[4pt]
...
% -------------------------------------------------------------------
%  SLIDE 150b — Segunda medida: saturación de la consigna de corriente
% -------------------------------------------------------------------
\figslide{Segunda medida: saturación de la consigna de corriente}{%
Se satura la consigna de corriente en el modulador de torque para no imponer desde el arranque una corriente que supere la corriente pico del motor. Aun así, el accionamiento \textbf{no} logra cumplir la consigna sin exceder sus especificaciones: la limitación por sí sola no basta y obliga a revisar la forma de la consigna.%
}{corrientes_comparacion.png}{Aun con saturación en la consigna de corriente, el accionamiento no puede cumplir la consigna sin superar sus especificaciones.}
...
\frac{d\theta_m(t)}{dt} = \omega_m(t),
\qquad
\frac{d\omega_m(t)}{dt} = \frac{1}{J_{eq}}\,T_m^{*\prime}(t),
\qquad
y(t) = \theta_m(t)
\end{equation*}
...
\begin{align*}
\frac{d\hat{\theta}_m}{dt} &= \hat{\omega}_m + K_{e\theta_m}\,(\theta_m - \hat{\theta}_m) \\[3pt]
\frac{d\hat{\omega}_m}{dt} &= \tfrac{1}{J_{eq}}\,T_m^{*\prime} + K_{e\omega_m}\,(\theta_m - \hat{\theta}_m)
\end{align*}
...
\begin{equation*}
\begin{bmatrix}\frac{de_\theta}{dt}\\[2pt] \frac{de_\omega}{dt}\end{bmatrix}
=
...
\begin{align*}
\frac{d\hat{\theta}_m}{dt} &= \hat{\omega}_m + K_{e\theta_m}\,e_\theta \\[3pt]
\frac{d\hat{\omega}_m}{dt} &= \tfrac{1}{J_{eq}}\,T_m^{*\prime} + K_{e\omega_m}\,e_\theta + K_{ei_m}\,z(t) \\[3pt]
\frac{dz(t)}{dt} &= e_\theta(t)
\end{align*}
...
\begin{equation*}
\frac{d\bm{e}_a}{dt} = \underbrace{\begin{bmatrix}
-K_{e\theta_m} & 1 & 0 \\
...
Implementación en Simulink del observador mecánico aumentado con acción integral. El error $e_\theta(t) = \theta_m(t) - \hat{\theta}_m(t)$ alimenta las ramas proporcionales y la rama integral; la salida integral corrige $\frac{d\hat{\omega}_m(t)}{dt}$ y elimina el error estacionario de estimación.%
}{5_2_5_B_diagrama_bloques_observador.png}{Diagrama de bloques del observador mecánico aumentado con acción integral implementado en Simulink. La rama integral corrige $\frac{d\hat{\omega}_m(t)}{dt}$ para eliminar el error estacionario.}
...
{\small\color{uncGray}Se evalúa la evolución de la temperatura del bobinado del estator $T_s^\circ(t)$ ciclo a ciclo bajo operación repetitiva, hasta detectar si supera el límite admisible.}\par
\vspace{3mm}
...
% -------------------------------------------------------------------
\figslide{Evolución térmica $T_s^\circ(t)$ bajo operación repetitiva}{%
...
\textbf{Resultado:}
\begin{itemize}
\item $T_s^\circ$ supera $115\,^\circ$C a $t = 225{,}9$~s ($\approx 15$ ciclos $+ 0{,}9$~s del ciclo 16).
...
$T_s^\circ$ & LP 1$^{er}$ orden & $\tau = 20$~s & $p = -1/\tau = -0{,}05$ \\
\hline
\end{tabular}
\normalsize
...
\textbf{Vector de estado} $\bm{x} = [\,x_1,\,x_2\,]^T = [\,y,\,\frac{dy}{dt}\,]^T$:
\begin{equation*}
\frac{dx_1}{dt} = x_2,
\qquad
...
\item Modulador de torque $\to$ $i_{qs}^{r*}$.
\item Modulador de corriente $\to$ $v_{qs}^{r*}$, $v_{ds}^{r*}$, $v_{0s}^{*}$.
\item Desacople de retroalimentaciones $\to$ $v_{qs}^{r}$, $v_{ds}^{r}$, $v_{0s}$.
\item Transformaciones de Park directa e inversa.
\end{itemize}
...
% -------------------------------------------------------------------
%  SLIDE — Conclusiones (I): modelado y control
% -------------------------------------------------------------------
\begin{frame}{Conclusiones (I): modelado y control}
\begin{itemize}\small
\item Del modelo \textbf{no lineal de 6 estados} en coordenadas $qd0$ se derivó un \textbf{LTI equivalente aumentado de 4 estados} por linealización por realimentación NL (ley mínima sobre $v_{ds}^{r}$ + ley complementaria sobre $v_{qs}^{r}$ que cancelan los acoplamientos electromagnéticos cruzados). La simulación comparativa verificó la equivalencia NL–LTI; la dinámica residual de $i_{ds}^{r}$ es transitoria, con $\tau_d = L_d/R_s \approx 6{,}24$~ms.
\item $i_{ds}^{r}$ resultó \textbf{no controlable} desde $v_{qs}^{r}$ y \textbf{no observable} desde $\theta_m$: justifica la cancelación polo–cero y habilita un \textbf{observador reducido} de la parte mecánica a partir del sensor de posición.
\item \textbf{Control en cascada:} modulador de torque con desacople completo y torque precalculado (gravitatorio y viscoso); lazos $P$ de corriente con polo en $-5000$~rad/s; PID externo por sintonía serie ($\zeta=0{,}75$, $\omega_n=800$~rad/s). El observador mecánico se ubicó con polos dobles en $-3200$~rad/s y luego debió \textbf{aumentarse con acción integral} (tres polos en $-3200$) para eliminar el offset de estimación ante perturbaciones de carga.
\end{itemize}
\end{frame}

% -------------------------------------------------------------------
%  SLIDE — Conclusiones (II): desempeño, discretización y mejoras
% -------------------------------------------------------------------
\begin{frame}{Conclusiones (II): desempeño, discretización y líneas de mejora}
\begin{itemize}\small
\item \textbf{Verificación de desempeño:} ante consignas con discontinuidades de aceleración/velocidad las corrientes exceden lo nominal $\Rightarrow$ generador de trayectorias con perfil trapezoidal de aceleración (\textbf{curva en S}). Bajo operación cíclica ($T_{amb}^\circ=25\,^\circ$C, $T_{ld}=5$~N$\cdot$m) la temperatura supera $115\,^\circ$C a $225{,}9$~s ($\approx 15$ ciclos): no cumple operación continua repetitiva. Modelando sensores e inversor como pasa-bajos, hubo que escalar los polos de sensores en $N=3$ y duplicar el ancho de banda del inversor a $12000$~rad/s.
\item \textbf{Discretización:} regla de Tustin con $T_s \leq 1{,}25\times 10^{-4}$~s (integradores del PID y del observador) y frontera digital–analógica con ZOH; la codificación modular en \textit{MATLAB Functions} resultó equivalente al diseño en bloques.
\item \textbf{Líneas de mejora:} campo orientado con máximo torque por amperio (MTPA); disipación térmica activa o reformulación del ciclo de trabajo; evaluación de robustez ante variación de $J_l$ y $b_l$; y validación experimental sobre plataforma física.
\end{itemize}
\end{frame}
