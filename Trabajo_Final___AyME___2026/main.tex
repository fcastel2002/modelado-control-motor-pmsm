...
\textbf{Ecuaciones de tensiones de fase referidas al rotor}
    ...
    \begin{equation}\label{eq:v0s_rotor}
        v_{0s}=R_s(T_s^\circ(t))i_{0s}(t)+L_{ls}\frac{d i_{0s}(t)}{dt}
    \end{equation}
    ...
    donde
    \begin{equation}
        P_{s,perd}(t)=\frac{3}{2}R_s\big(T_s^\circ(t)\big)\Big(i_{qs}^{r\,2}(t)+i_{ds}^{r\,2}(t)+2i_{0s}^{r\,2}(t)\Big)
    \end{equation}
...
\left\{
    \begin{aligned}
        \frac{d\theta_{m,o}}{dt}(t) &= \omega_{m,o}(t) = \text{cte} \\[10pt]
        %
        \frac{d\omega_{m,o}}{dt}(t) &= \frac{1}{J_{eq}} \left[ -b_{eq}\omega_{m,o}(t) + \frac{3}{2}P_p \left[ \lambda'^r_m + (L_d - L_q)i^r_{ds,o}(t) \right] i^r_{qs,o}(t) - g \cdot k_l \sin\left(\frac{\theta_{m,o}}{r}\right) - \frac{1}{r}T_{ld}(t) \right] = 0 \\[10pt]
        %
        \frac{di^r_{qs,o}}{dt}(t) &= \frac{1}{L_q} \left[ v^r_{qs,o}(t) - R_s(T_{s,o}^\circ(t))i^r_{qs,o}(t) - (\lambda'^r_m + L_d i^r_{ds,o}(t))P_p \omega_{m,o}(t) \right] = 0 \\[10pt]
        %
        \frac{di^r_{ds,o}}{dt}(t) &= \frac{1}{L_d} \left[ v^r_{ds,o}(t) - R_s(T_{s,o}^\circ(t))i^r_{ds,o}(t) + L_q i^r_{qs,o}(t)P_p \omega_{m,o}(t) \right] = 0 \\[10pt]
        %
        \frac{di_{0s,o}}{dt}(t) &= \frac{1}{L_{ls}} \left( v_{0s,o}(t) - R_s(T_{s,o}^\circ(t))i_{0s,o}(t) \right) = 0 \\[10pt]
        %
        \frac{dT_{s,o}^\circ}{dt}(t) &= \frac{1}{C_{ts}} \left[ \frac{3}{2}R_s(t) \left( i^r_{qs,o}(t)^2 + i^r_{ds,o}(t)^2 + 2i_{0s,o}(t)^2 \right) - \frac{1}{R_{ts\text{-}amb}} (T_{s,o}^\circ(t) - T_{amb,o}^\circ(t)) \right] = 0
    \end{aligned}
\right.
\end{equation}
...
\left\{
    \begin{aligned}
        ...
        %
        \Delta \dot{i}_{0s}(t) &= \frac{1}{L_{ls}} \left[ \Delta v_{0s}(t) - R_{s,o} \cdot \Delta i_{0s}(t) - R_{sREF} \alpha_{Cu} \Delta T_s^\circ(t) \cdot i_{0s} \right] \\[10pt]
        %
...
0 & -\frac{P_p (\lambda'^r_m + L_d i^r_{ds,o})}{L_q} & -\frac{R_s}{L_q} & -\frac{L_d P_p \omega_{m,o}}{L_q} & 0 & -\frac{R_{sREF} \alpha_{Cu} i^r_{qs,o}}{L_q} \\
0 & \frac{L_q P_p i^r_{qs,o}}{L_d} & \frac{L_q P_p \omega_{m,o}}{L_d} & -\frac{R_s}{L_d} & 0 & -\frac{R_{sREF} \alpha_{Cu} i^r_{ds,o}}{L_d} \\
0 & 0 & 0 & 0 & -\frac{R_s}{L_{ls}} & -\frac{R_{sREF} \alpha_{Cu} i_{0s,o}}{L_{ls}} \\
0 & 0 & \frac{3 R_s i^r_{qs,o}}{C_{ts}} & \frac{3 R_s i^r_{ds,o}}{C_{ts}} & \frac{6 R_s i_{0s,o}}{C_{ts}} & \Gamma_{th,o}
\end{bmatrix}
\end{equation*}
...
\begin{enumerate}
    ...
    Por lo que, podemos decir que el Modelo LTI Aumentado puede interpretarse estructuralmente como el caso ideal de un modelo LPV forzado perpetuamente a operar sobre el punto $I^r_{ds,o} = 0$, habiendo agrupado el torque gravitatorio dentro de la perturbación de entrada $T_l(t)$.
\end{enumerate}
...
Para evaluar el impacto dinámico, se analiza la migración de los polos del sistema LPV (los autovalores de la matriz $\mathbf{A}_0$). Se fija un punto de operación nominal de velocidad ($\omega_{m0}$) y torque ($I^r_{qs,o}$), y se realiza un barrido del parámetro $I^r_{ds,o}$ desde la zona de debilitamiento hasta la zona de reforzamiento de campo. 
...
\begin{enumerate}
    \item \textbf{Frecuencia de oscilación:} La variación de $I^r_{ds,o}$ afecta drásticamente a los términos cruzados electromecánicos en la matriz $\mathbf{A}_0$. Esto se traduce en una variación significativa de la parte imaginaria de los polos dominantes complejos conjugados. A medida que se pasa de debilitamiento a reforzamiento (aumenta $I^r_{ds,o}$), la frecuencia natural de oscilación del sistema aumenta.
...
%     \caption{Mapa de migración de polos del modelo LPV variando $I^r_{ds,o}$.}
...
La resistencia del estator varía con la temperatura según $R_s\big(T_s^\circ(t)\big)=R_{sREF}\big[1+\alpha_{Cu}\big(T_s^\circ(t)-T_{sREF}^\circ\big)\big]$. Para evaluar su efecto sobre la dinámica del modelo LTI, se realiza un barrido alrededor del valor de operación $R_{s0}$, en el rango $[0{,}8\,R_{s0},\; 1{,}2\,R_{s0}]$.
...
\begin{figure}[H]
    ...
    \caption{Error de seguimiento en $\theta_m(t)$: diferencia $\theta_m^{NL}-\theta_m^{LTI}$, acotada por debajo de $0{,}1$~rad ($<0{,}05\%$ de la excursión de $242$~rad).}
...
\begin{figure}[H]
    ...
    \caption{Error de seguimiento en $\omega_m(t)$: diferencia $\omega_m^{NL}-\omega_m^{LTI}$, nula en régimen y con picos $\leq 3$~rad/s únicamente en las conmutaciones.}
...
\begin{figure}[H]
    ...
    \caption{Error de seguimiento en $i_{qs}^r(t)$: diferencia $i_{qs}^{NL}-i_{qs}^{LTI}$, nula en régimen y con picos $\leq 0{,}1$~A únicamente en las conmutaciones.}
...
\begin{figure}[H]
    ...
    \caption{Tensiones de fase $v_a$, $v_b$, $v_c$ aplicadas al estator (modelo NL, $i_{ds}^r(0)=0$~A), con el mismo código de colores por fase que las corrientes.}
...
\begin{align}
    V_{\alpha} = \sqrt{2}\,\tfrac{V_{sl}}{\sqrt{3}} \cos{\theta_{ev}} \\
    V_{\beta} = \sqrt{2}\,\tfrac{V_{sl}}{\sqrt{3}} \sin{\theta_{ev}}
\end{align}
Por lo tanto, para obtener el ángulo $\theta_{ev}$ basta con aplicar la función \textit{atan2} tal que:
\begin{equation}
    \theta_{ev} = \text{atan2}(V_{\beta},V_{\alpha})= \text{atan2}\big(\sqrt{2}\,\tfrac{V_{sl}}{\sqrt{3}} \sin{\theta_{ev}},\ \sqrt{2}\,\tfrac{V_{sl}}{\sqrt{3}} \cos{\theta_{ev}}\big)
\end{equation}
...
\begin{itemize}
    ...
    \item En el \textbf{modelo NL}, $i_{ds}^r(t)$ también decae exponencialmente con la misma constante de tiempo. Aunque existe un acoplamiento residual NL del tipo $\omega_r \cdot L_q \cdot i_{qs}^r$, la ley de control mínima ($v_{ds} = -L_q i_{qs}^r \omega_r$) lo cancela exactamente, y la ley complementaria ($v_{qs} = v_{qs}^{r*} + \omega_r L_d i_{ds}^r$) elimina el efecto cruzado inverso.
...
\begin{equation}
    v_{ds}(t) = \underbrace{-L_q \, i_{qs}^r \, \omega_r}_{\text{ley mínima}} + v_{ds}^{r*}(t), \quad
...
\textbf{Análisis:}
\begin{itemize}
    \item En el \textbf{modelo LTI}, $v_{ds}^{r*}$ afecta \textbf{únicamente} a $i_{ds}^r$, que crece exponencialmente hasta su valor estacionario $i_{ds,ss}^r = v_{ds}^{r*}/R_{s0} = \pm 1{,}92$~A. Los demás estados ($\theta_m$, $\omega_m$, $i_{qs}^r$) permanecen \textbf{inalterados} por el desacoplamiento estructural de la matriz $A_{aug}$.
...
\end{equation}
\begin{equation}
    v_{0s}^{r*}(t) = L_{ls} \frac{di_{0s}(t)}{dt}
    \label{eq:v0s_ref}
\end{equation}
\begin{figure}[H]
    \centering
...
v_{0s}^{r*}(t) &= K_{p0} \left(i_{0s}^{r*}(t) - i_{0s}(t)\right)
\end{aligned}
\end{equation*}
...
Queremos los polos en $p_i = -5000 \ [\frac{rad}{s}]$, por lo tanto las ganancias $K_{p}^{qd0}$ quedarían: \[K_{p}^{qd0}=L_{q,d,ls}\cdot 5000 \ \Omega\]
...
K_{e\theta_m}=6400,
\qquad
K_{e\omega_m}=3200^2=1.024\times10^7.
\end{equation}
...
K_{e\theta_m}=9600,
\qquad
K_{e\omega_m}=3.072\times10^7,
\qquad
K_{ei_m}=3.2768\times10^{10}.
\end{equation}
...
El modulador de corriente constituye el lazo interno del controlador. Este bloque compara las corrientes de referencia $i_{qs}^{*}$, $i_{ds}^{*}$ e $i_{0s}^{*}$ con las corrientes medidas en coordenadas $qd0$, generando las tensiones de control $v_{qs}^{r*}$, $v_{ds}^{r*}$ y $v_{0s}^{*}$. Estas tensiones son todavía señales de control ideales, por lo que luego se corrigen mediante el bloque de desacople.
