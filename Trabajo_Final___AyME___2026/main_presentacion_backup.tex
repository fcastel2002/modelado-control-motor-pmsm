...
$\lambda'_m$  & Flujo de imanes             & $0{,}016$~Wb \\
...
Extrae $\sin(\theta_m/r)$ \emph{completo} como perturbación externa en $\bm{B}_d^{2}$. La matriz $A_{2,1}=0$ permanece \textbf{invariante}. \\
\hline
Subsistema térmico &
...
Caso particular: el acoplamiento cruzado se vuelve puramente $\lambda'_m$ (sin reluctancia), coincidiendo con $A_{3,2}$ del LTI. &
\textbf{Es} el caso ideal de un LPV operando perpetuamente en $I_{ds0}^{r}=0$ con la gravedad extraída como perturbación. \\
\hline
\end{tabular}%
}
...
}{sim_iqs.png}{Corriente de eje $q$, $i_{qs}(t)$: comparación NL (azul) vs LTI aumentado (rojo). Panel inferior: señales de excitación ($T_l$ y $v_{qs}^*$).}
...
}{sim_iqs_error.png}{Error de seguimiento en $i_{qs}(t)$: diferencia $i_{qs}^{NL}-i_{qs}^{LTI}$. \textbf{Error nulo en régimen; picos $\leq 2{,}5\%$ del pico de $\pm 10{,}4$~A en conmutaciones.}}
...
\vspace{2mm}
\textbf{Resto del ensayo invariante:} mismas señales de excitación $v_{qs}^{*}(t)$ y $T_{ld}(t)$, misma ley de control mínima $v_{ds} = -L_q\,i_{qs}\,\omega_r$, mismo $T_{amb}=25\,^\circ$C.
...
\begin{itemize}
\item \textbf{$i_{ds}^{r} = 0$ (estrategia base):} el torque depende únicamente de $i_{qs}^{r}$ y del flujo de los imanes $\lambda'_m$. Suficiente para la mayoría de aplicaciones y para el alcance de este proyecto.
