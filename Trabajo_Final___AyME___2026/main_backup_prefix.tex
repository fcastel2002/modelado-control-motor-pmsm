...
\subsection{Máquina PMSM}
    ...
    \end{equation}
    Despejando \(T_d(t)\) y reemplazando \(T_l(t)\) por la Ec. 2:
...
\begin{figure}[H]
    ...
    \caption{Respuesta dinámica al escalón de tensión $v_q=5$V. \textbf{Arriba:} Corriente de estator $i_q$ [A], mostrando el transitorio inductivo y el efecto de la fuerza contraelectromotriz. \textbf{Abajo:} Velocidad mecánica del rotor $\omega_m$ [rad/s].}
...
Considerando que el modelo actual es de la forma \[\bm{\dot x} =\bm{f}(\bm{x},u)\]
donde primero se deben encontrar los puntos de equilibrio del sistema, es decir, los valores \(\bm{x}_0\) y \(u_0\) tales que \(\bm{\dot x_0} = \bm{f}(\bm{x}_0,u_0)\). Definiendo \(x=x_0+\delta x\) y \(u=u_0+\delta u\), se expande la ecuación no lineal en términos de las perturbaciones respecto a estos valores de equilibrio, lo que resulta en:
\begin{equation*}
...
Primero, se encuentra el espacio de operación global no lineal, que es el espacio donde se resolverá la primera parte de la ecuación expandida, $\bm{f}(\bm{x}_0,u_0)=0$.
...
\begin{cases}
    ...
    \vspace{0.2cm}
    \dfrac{d\omega_m(t)}{dt} = \dfrac{1}{J_{eq}}\Big[\frac{3 P_p \lambda'_m}{2} i_{qs}(t) - b_{eq} \omega_m(t) - \frac{1}{r} T_l(t)\Big] \\
...
Definiendo el vector de estados nominal $\mathbf{x}_{LTI} = [\theta_m, \omega_m, i_{qs}]^T$, la representación matricial de estado resulta:
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textit{Modelo LTI Aumentado:} El torque gravitacional completo se agrupa con el torque externo de carga en la perturbación total $T_l(t)=T_{ld}(t)+g\,k_l\sin(\theta_m/r)$. En consecuencia, el término seno no se deriva ni ingresa en la matriz $\mathbf{A}$; la matriz de estado del LTI nominal ($A_{2,1}=0$) permanece invariante frente a la posición.
    \end{itemize}
    ...
    Por lo que, podemos decir que el Modelo LTI Aumentado puede interpretarse estructuralmente como el caso ideal de un modelo LPV forzado perpetuamente a operar sobre el punto $I_{ds0}^r = 0$, habiendo agrupado el torque gravitatorio dentro de la perturbación de entrada $T_l(t)$.
\end{enumerate}
...
A partir del modelo LTI equivalente aumentado de cuarto orden, se obtienen las funciones de transferencia desde ambas entradas ---la acción de control $v_{qs}(t)$ y la perturbación mecánica total $T_l(t)$--- hacia la salida de posición $\theta_m(t)$.
...
\end{equation}
Además, dado que el torque gravitatorio fue agrupado dentro de la perturbación total $T_l(t)$, no aparece ningún término no lineal dependiente de $\sin(\theta_m/r)$ en la matriz de estado. La resistencia del estator se considera constante ($R_s = R_{s0}$) por el desacoplamiento térmico.
...
\begin{cases}
    ...
    s\,\Omega_m(s) = \dfrac{1}{J_{eq}}\left[\dfrac{3 P_p \lambda'_m}{2} I_{qs}(s) - b_{eq}\,\Omega_m(s) - \dfrac{1}{r}\,T_l(s)\right] \\[6pt]
...
\begin{equation}
    \left(J_{eq}\,s + b_{eq}\right)\Omega_m(s) = \frac{3 P_p \lambda'_m}{2} \cdot \frac{V_{qs}(s) - P_p \lambda'_m\,\Omega_m(s)}{L_q\,s + R_{s0}} - \frac{1}{r}\,T_l(s)
\end{equation}

Multiplicando ambos miembros por $(L_q\,s + R_{s0})$ y reordenando:
\begin{equation}
    \left[(J_{eq}\,s + b_{eq})(L_q\,s + R_{s0}) + \frac{3 P_p^2 \lambda_m'^2}{2}\right]\Omega_m(s) = \frac{3 P_p \lambda'_m}{2}\,V_{qs}(s) - \frac{L_q\,s + R_{s0}}{r}\,T_l(s)
\end{equation}
...
\boxed{
    G_2(s) = \frac{\theta_m(s)}{T_l(s)} = \frac{-\dfrac{1}{r J_{eq}}\left(s + \dfrac{R_{s0}}{L_q}\right)}{s^3 + \left(\dfrac{b_{eq}}{J_{eq}} + \dfrac{R_{s0}}{L_q}\right)s^2 + \left(\dfrac{b_{eq} R_{s0}}{J_{eq} L_q} + \dfrac{3 P_p^2 \lambda_m'^2}{2 J_{eq} L_q}\right)s}
}
\end{equation}
...
\begin{figure}[H]
    ...
    \caption{Diagrama de bloques en Simulink: modelo NL desacoplado y LTI aumentado en paralelo frente a la misma consigna $v_q^*$ y al mismo torque de perturbación.}
...
El ángulo de carga $\delta(t)$ representa el desfasaje electromagnético entre el ángulo eléctrico $\theta_{ev}$ y la posición angular del rotor en coordenadas eléctricas $\theta_r$. Como referencia eléctrica se utiliza la posición del voltaje $V_{as}$, que es constante en estado estacionario.
...
\begin{itemize}
    \item \textbf{Tensión $v_{qs}^*(t)$}: es la acción \textbf{dominante}. Fija la velocidad de vacío del motor (donde la fuerza contraelectromotriz equilibra la tensión aplicada) y produce el pico de corriente/torque ($\approx \pm 10{,}4$~A); la respuesta de $\omega_m$ es subamortiguada (sobrepico $\approx 14\%$).
...
Recordando las ecuaciones \eqref{eq:vqs_rotor}, \eqref{eq:vds_rotor} y \eqref{eq:v0s_rotor}:
    ...
    \begin{equation*}
        v^r_{0s}=R_s(T_s^\circ(t))i^r_{0s}(t)+L_{ls}\frac{d i^r_{0s}(t)}{dt}
    \end{equation*}
Se observan las retroalimentaciones naturales: la dependencia de la resistencia variable con la temperatura $T_s^\circ(t)$, el acoplamiento entre las corrientes $i_{qs}$ e $i_{ds}$, y las demás fuentes de no linealidad ya estudiadas anteriormente en el presente trabajo.
...
\end{gather*}
Puede observarse en la ecuación anterior que la variable $i_{ds}$ quedaría como un parámetro más, lo que habilita la posibilidad de dos casos ya vistos anteriormente en el presente trabajo.
\begin{itemize}
    \item $i_{ds} = 0$: si se aplica esta consigna, el torque depende únicamente de la corriente en eje $q$ y del flujo de los imanes permanentes, lo que resulta suficiente para la mayoría de los casos.
    \item $i_{ds} \neq 0$: ya sea por reforzamiento o debilitamiento de campo, $i_{ds}$ actuaría como una consigna adicional para situaciones en las que el rango de voltaje del inversor no sea suficientemente amplio.
\end{itemize}
\begin{figure}[H]
    \centering
...
\end{figure}
Como puede verse en la Fig.~\ref{fig:corrientes_voltaje}, aunque se reduce bastante el tamaño del pico de corriente y no cambia el error de posición en la simulación con tensión saturada, esto no es suficiente para cumplir con las especificaciones de corriente.
...
\end{figure}
Es interesante observar en la Fig.~\ref{fig:sens_pos_1} cómo, con los polos elegidos para el modelo, los sensores no son lo suficientemente rápidos respecto a la dinámica del lazo de corriente, lo que era de esperarse: si se recuerda el diseño del modulador de corriente, se habían elegido $p_{a,b,c}=-5000 \ [rad/s]$, por lo que modelar los sensores de corriente con $p_{sens,abc}=-6000 \ [rad/s]$ no resulta lo suficientemente más rápido que la dinámica del sistema. Lo mismo ocurre con los polos del observador. Por eso se propone aumentar estos polos de forma conjunta, es decir, $\omega_n^{\prime} = N \cdot \omega_n;\ N=1,2,3\ldots$
