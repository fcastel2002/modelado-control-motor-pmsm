...
con $V_m = \sqrt{\tfrac{2}{3}}\,V_{sl}$, siendo $V_{sl}$ el \textbf{módulo de tensión de línea}.
...
\begin{equation*}
\bm{u}_{NL}(t) = \underbrace{\big[\,v_{qs}^{r},\;v_{ds}^{r},\;v_{0s}^{r}\,\big]^T}_{\text{manipuladas (vía Park del inversor)}} \;\;\cup\;\; \underbrace{\big[\,T_{ld},\;T_{amb}^\circ,\ g\,\big]^T}_{\text{perturbaciónes externas}}
\end{equation*}
...
\vspace{1mm}
{\small \textbf{Con} $T_m = \tfrac{3}{2}P_p\lambda'^r_m\,i_{qs}^{r} + \tfrac{3}{2}P_p(L_d-L_q)\,i_{ds}^{r}\,i_{qs}^{r}$ \;y\; $\omega_r = P_p\omega_m$.\quad \textbf{No linealidades:} (i) gravitatoria $\sin(\theta_m/r)$; (ii) producto $i_{ds}^{r}\!\cdot\! i_{qs}^{r}$ en $T_m$; (iii) cuadráticas en $P_{Cu}$; (iv) $R_s(T_s^\circ)$.}
\end{frame}
...
\begin{equation*}
\rho=\big(\theta_{mO},\;\omega_{mO},\;i_{qsO}^{r},\;i_{dsO}^{r},i^r_{0sO},\;T_{sO}^\circ\big)
\end{equation*}
...
0 & 1 & 0 & 0 & 0 & 0 \\[2pt]
-\dfrac{g\,k_l\cos\!\big(\theta_{mO}/r\big)}{J_{eq}\,r^2} & -\dfrac{b_{eq}}{J_{eq}} & \dfrac{3 P_p\!\left[\lambda'^r_m + (L_d{-}L_q)\,i_{dsO}^{r}\right]}{2\,J_{eq}} & \dfrac{3 P_p (L_d{-}L_q)\,i_{qsO}^{r}}{2\,J_{eq}} & 0 & 0 \\[6pt]
0 & -\dfrac{P_p\!\left[\lambda'^r_m + L_d\,i_{dsO}^{r}\right]}{L_q} & -\dfrac{R_{sO}}{L_q} & -\dfrac{L_d P_p\,\omega_{mO}}{L_q} & 0 & -\dfrac{R_{sREF}\,\alpha_{Cu}\,i_{qsO}^{r}}{L_q} \\[6pt]
...
{\small\color{uncGray}Aplicando la ley mínima en el eje $d$ y fijando $R_s=R_{s0}$ constante, la planta electromecánica principal queda reducida a tres estados dinámicos. El torque gravitatorio se agrupa con la carga externa en una perturbación mecánica total.}\par
\vspace{2mm}

\textbf{Perturbación mecánica total} (salvedad ingenieril):
\begin{equation*}
...
{\small El término gravitatorio no se deriva ni ingresa a la matriz de estado: se trata como parte de la entrada perturbadora $T_l(t)$, conservando una matriz $\bm{A}$ lineal e invariante en el tiempo.}
\end{frame}
...
\begin{itemize}
  ...
  \item $\bm{B}_d$ inyecta la perturbación mecánica total $T_l = T_{ld} + g\,k_l\sin(\theta_m/r)$.
\end{itemize}
...
El diagrama resume la estructura final del modelo nominal: la corriente $i_{qs}^r$ genera torque, la velocidad realimenta por FCEM y las perturbaciónes mecánicas ingresan por canales separados. Sobre esta planta se apoyarán el modelo aumentado y los análisis posteriores a lazo abierto.%
...
{\small\color{uncGray}La estructura matricial deja ver que $i_{ds}^{r}$ evolucióna como un modo desacoplado de primer orden, mientras la terna $(\theta_m,\omega_m,i_{qs}^r)$ conserva la misma planta nominal.}\par
\vspace{2mm}
...
Agrupa el torque gravitacional \emph{completo} con la carga externa en la perturbación total $T_l(t)=T_{ld}(t)+g\,k_l\sin(\theta_m/r)$. La matriz $A_{2,1}=0$ permanece \textbf{invariante}. \\
\hline
Subsistema térmico &
...
\hline
Estructura al forzar $I_{ds0}^{r}\equiv 0$ &
Caso particular: el acoplamiento cruzado se vuelve puramente $\lambda'^r_m$ (sin reluctancia), coincidiendo con $A_{3,2}$ del LTI. &
\textbf{Es} el caso ideal de un LPV operando perpetuamente en $I_{ds0}^{r}=0$, habiendo agrupado el torque gravitatorio dentro de la perturbación de entrada $T_l(t)$. \\
\hline
\end{tabular}%
}

\vspace{2mm}
{\scriptsize \textbf{Conclusión:} ambos modelos son comparables pero no equivalentes en su origen. El LTI aumentado puede interpretarse como el caso ideal del LPV con $I_{ds0}^{r}\equiv 0$ y la gravedad tratada como entrada exógena.}
\end{frame}
...
\begin{equation*}
T_m=\frac{3}{2}P_p\Big[\lambda'^r_m+(L_d-L_q)I_{dsO}^{r}\Big]I_{qsO}^{r}
\end{equation*}
...
}{polosaliente_cilindrico.png}{Diagrama representativo de un rotor genérico de (c)(d)polos no salientes (arriba) y un (a)(b)rotor de polos salientes (abajo).}
...
% -------------------------------------------------------------------
\figslide{Mapa de migración de polos con $I_{dsO}^{r}$ --- zoom}{%
...
{\small\color{uncGray}A partir del modelo LTI aumentado se buscan las relaciones entrada-salida desde la acción de control $v_{qs}$ y la perturbación mecánica total $T_l$ hacia la posición $\theta_m$.}\par
\vspace{3mm}
...
\begin{itemize}
  ...
  \item El torque gravitatorio está agrupado dentro de la perturbación total $T_l(t)$: no aparece ningún término no lineal en la matriz $\bm{A}_{aug}$.
...
{\small\color{uncGray}El polinomio característico del modelo aumentado separa el integrador cinemático de posición, el modo electromecánico y el modo residual del eje $d$.}\par
\vspace{1mm}
...
Pico de torque transitorio $\approx \pm 10{,}4$~A (no $v_{qs}^{*}/R_{s0}$) que decae hasta $\approx 0{,}1$~A en régimen, a medida que la FCEM $\lambda'^r_m P_p \omega_m$ equilibra la tensión aplicada. El pico es el valor relevante frente a $I_{s,max} = 2$~A rms (límite ampliamente superado en este ensayo LA). Como $i_{ds}^{r} \equiv 0$, $T_{em} = k_t\,i_{qs}^r$ con $k_t = \tfrac{3}{2}P_p\lambda'^r_m$: la forma de onda de $T_{em}(t)$ es proporcional a $i_{qs}^r(t)$.%
}{sim_iqs.png}{Corriente de eje $q$, $i_{qs}^r(t)$: comparación NL (azul) vs LTI aumentado (rojo). Panel inferior: señales de excitación ($T_l$ y $v_{qs}^*$).}
...
}{sim_iqs_error.png}{Error de seguimiento en $i_{qs}^r(t)$: diferencia $i_{qs}^{NL}-i_{qs}^{LTI}$. \textbf{Error nulo en régimen; picos $\leq 2{,}5\%$ del pico de $\pm 10{,}4$~A en conmutaciones.}}
...
La ley de control mínima impone internamente $v_{ds}=-L_q\,i_{qs}^r\,\omega_r$, proporcional al producto $i_{qs}^r\cdot\omega_m$. Esta tensión es no nula solo cuando ambas variables lo son simultáneamente, y su magnitud crece con el nivel de operación del sistema.%
...
\vspace{2mm}
\textbf{Resto del ensayo invariante:} mismas señales de excitación $v_{qs}^{*}(t)$ y $T_{ld}(t)$, misma ley de control mínima $v_{ds} = -L_q\,i_{qs}^r\,\omega_r$, mismo $T_{amb}=25\,^\circ$C.
...
% -------------------------------------------------------------------
\figslide{$i_{ds}^{r}(t)$ --- modelo NL y modelo LTI, tres condiciones iniciales}{%
...
}{ids_distintodecero_simulink.png}{Comparación de $i_{ds}^r(t)$ para $i_{ds}^r(0) \in \{0,\,+0{,}5,\,-0{,}5\}$~A: modelo NL y modelo LTI.}

% -------------------------------------------------------------------
%  SLIDE 96 — i_ds LTI 3 casos
% -------------------------------------------------------------------
\figslide{$i_{ds}^{r}(t)$ --- error modelo NL frente a modelo LTI}{%
En el LTI las trayectorias son exponenciales puras de primer orden. Coinciden cualitativamente con las del modelo NL porque la ley de control mínima cancela el acoplamiento cruzado residual.%
}{ids_distintodecero_simulink_error.png}{Comparación de $i_{ds}^r(t)$ de modelo NL vs modelo LTI para $i_{ds}^r(0) +0{,}5$~A}
...
\begin{equation*}
v_{ds}(t) = \underbrace{-L_q\,i_{qs}^r\,\omega_r}_{\text{ley mínima}} + v_{ds}^{*}(t)
\end{equation*}
...
{\small\color{uncGray}El modelo no lineal revela los acoplamientos cruzados que el LTI no representa: la dinámica del eje $d$ depende de $i_{qs}^r\omega_m$, y el torque incorpora el término de reluctancia $(L_d-L_q)i_{ds}^{r}$.}\par
\vspace{3mm}
...
}{desacop_realimentacion_fisicas_naturales.png}{Implementación en Simulink del desacoplamiento de las retroalimentaciones físicas naturales: cancelación de caídas resistivas, FCEM y acoplamientos cruzados entre ejes $q$ y $d$.}
...
Evolución temporal del torque neto $T_{\text{neto}}$ [N$\cdot$m]. Se contrasta el comportamiento con resistencia estatórica constante (RS Cte., línea roja) y con resistencia estatórica variable (RS Variable, línea azul).%
}{comp_torque_rs.png}{Evolución temporal del torque neto $T_{\text{neto}}$~[N$\cdot$m]: comparación con $R_s$ constante (rojo) y $R_s$ variable (azul).Déficit de torque $\sim 15\%$}
...
% -------------------------------------------------------------------
%  SLIDE 119 — Método de sintonía serie
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  SLIDE 120 — Relaciones entre frecuencias y elección de omega_pos
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  SLIDE 121 — Despeje de las ganancias en función de n
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  SLIDE 122 — Polinomio característico y factorización
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  SLIDE 124 — Valores numéricos de ganancias
% -------------------------------------------------------------------
