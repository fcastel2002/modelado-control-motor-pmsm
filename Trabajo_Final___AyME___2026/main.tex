...
donde $\omega_l(t)$ y $\theta_l(t)$ representan la velocidad y la posición angular de la articulación, respectivamente; $T_q(t)$ es el torque impulsor entregado por el reductor; $T_{ld}(t)$ modela las perturbaciones externas variables; $J_l$ y $b_l$ son la inercia total y el coeficiente de fricción viscosa del eje de carga; y $T_l(t)$ es el torque de carga resistente, definido en la Ec.~\eqref{eq:def_Tl} por el término gravitatorio ---de coeficiente $k_l$--- más la perturbación $T_{ld}(t)$. Los valores numéricos de los parámetros físicos constantes que caracterizan al péndulo y la transmisión se detallan en la Tabla~\ref{tab:parametros}.

Siguiendo la nomenclatura de la guía de trabajo~\cite{guia_trabajo}: la coordenada articular $q(t) \equiv \theta_l(t)$ ---referida al eje vertical hacia abajo (posición de equilibrio estable) y positiva en sentido antihorario--- es la \textbf{variable controlada} del accionamiento, no medida directamente, con $q(t)=\theta_l(t)=\frac{1}{r}\,\theta_m(t)$, siendo $\theta_m(t)$ la posición angular del eje del motor. En todo el informe, el subíndice $l$ (del inglés \textit{load}) designa las magnitudes referidas al eje de la carga (articulación) y el subíndice $m$, las del eje del motor. El torque impulsor sobre el eje de carga se denota indistintamente $\tau(t) \equiv T_q(t)$. La aceleración de la gravedad $g$ actúa como perturbación externa vertical constante (hacia abajo).
...
\end{align}
donde $\theta_{ev}(t)$ es la posición angular eléctrica (fase instantánea) de la tensión sintetizada por el inversor, cuya derivada es la frecuencia angular eléctrica $\omega_e(t)$. Por lo que definiremos:
...
\end{bmatrix}
\end{equation}
donde $\theta_r(t)$ es la posición eléctrica del rotor definida previamente.
...
\textbf{Ecuaciones de tensiones de fase referidas al rotor.} Aplicando la transformación directa \eqref{eq:park_directa} al vector de tensiones de fase $\bm{\mathrm{v}}_{abcs}$ se obtienen las tensiones en coordenadas rotóricas $\bm{\mathrm{v}}^{r}_{qd0s}=(v^r_{qs},\,v^r_{ds},\,v_{0s})$, cuyo balance eléctrico es:
    ...
    \[
        \bm{u}_{NL}(t)=\begin{bmatrix}
            v_{qs}^r(t) & v_{ds}^r(t) & v_{0s}(t) & T_{ld}(t) & T_{amb}^\circ(t) & g
        \end{bmatrix}^T
    \]
    Se incorpora la aceleración de la gravedad $g$ al vector de entradas: si bien en cada escenario de operación es constante, tratarla como entrada externa (parámetro del entorno) generaliza el modelo a distintos campos gravitatorios ---por ejemplo, operar el manipulador en otro planeta---. Su efecto ingresa a la dinámica a través del torque gravitatorio $g\,k_l\sin(\theta_m/r)$, y en las linealizaciones posteriores se mantiene constante ($\Delta g = 0$), por lo que su influencia queda contenida en el punto de operación.
...
Se implementó el modelo no lineal completo del accionamiento en el entorno MATLAB/Simulink, estructurando la planta mediante bloques funcionales interconectados que representan las ecuaciones diferenciales deducidas previamente (Fig.~\ref{fig:simulink_top}). La planta recibe como entradas las tensiones de fase $v_{abcs}(t)$ sintetizadas por el inversor y el torque de perturbación $T_{ld}(t)$, y entrega como salidas la posición y la velocidad del rotor ($\theta_m$, $\omega_m$), las corrientes de fase $i_{abcs}(t)$, la temperatura del estator $T_s^\circ$ y el torque electromagnético $T_m$.

La interconexión de bloques reproduce directamente la estructura del modelo físico deducido:
\begin{itemize}
    \item La \textbf{Transformación de Park} lleva las tensiones de fase $abc$ al marco rotórico $qd0$ ($v_{qd0s}^r$), en el que operan las ecuaciones eléctricas de la máquina, empleando la posición eléctrica $\theta_r = P_p\,\theta_m$.
    \item El \textbf{Subsistema Electromagnético} calcula las corrientes $i_{qd0s}^r$ y el torque electromagnético $T_m$ a partir de dichas tensiones, la velocidad $\omega_m$ y la resistencia $R_s$.
    \item El \textbf{Subsistema Mecánico} integra la dinámica de $\theta_m$ y $\omega_m$ a partir de $T_m$ y $T_{ld}$, mientras que el \textbf{Subsistema Térmico} obtiene la temperatura $T_s^\circ$ y la resistencia $R_s(T_s^\circ)$ a partir de las pérdidas resistivas, cerrando el acoplamiento con el subsistema electromagnético.
    \item La \textbf{Transformación Inversa de Park} reconvierte las corrientes $i_{qd0s}^r$ al marco de fase $abc$ para su lectura.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Diagrama de bloques del modelo del accionamiento implementado en Simulink. Las Transformaciones de Park directa e inversa vinculan el marco de fase $abc$ con el marco rotórico $qd0$, y se muestra la interconexión de los subsistemas electromagnético, mecánico y térmico.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{imagenes/migracion_ids_1.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{imagenes/migracion_ids_2.png}
...
\end{equation}
La cuarta columna de $\mathcal{O}$ es idénticamente nula porque la cuarta fila de $\bm{A}_{aug}$ es diagonal (el estado $i_{ds}^r$ no se acopla con ningún otro) y $C(4)=0$; esto vale para toda potencia $\bm{C}\bm{A}^k$. Por lo tanto:
\begin{equation}
...
Si en cambio se mide la velocidad angular, la matriz de salida es $\bm{C}_\omega = \begin{bmatrix} 0 & 1 & 0 & 0 \end{bmatrix}$. En $\mathcal{O}_\omega$ se anulan dos columnas: la cuarta (por el desacople de $i_{ds}^r$, ya discutido) y también la primera, ya que la posición $\theta_m$ solo se acopla unidireccionalmente como integrador de $\omega_m$ y no aparece en ninguna potencia $\bm{C}_\omega\bm{A}^k$.
...
\end{equation}
La cuarta fila de $\mathcal{C}$ es idénticamente nula: la entrada $v_{qs}$ nunca llega a $i_{ds}^r$ ($B_c(4)=0$) y, al ser diagonal la cuarta fila de $\bm{A}$, ningún otro estado lo excita. Por lo tanto:
\begin{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/sim_senales_entrada.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/sim_theta_m.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/sim_theta_m_error.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/sim_omega_m.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/sim_omega_m_error.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/sim_iqs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/sim_iqs_error.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/sim_Ts.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/sim_v_ds.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/sim_corrientes_abc.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/sim_tensiones_abc.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/sim_torque_vs_velocidad.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/sim_angulo_carga.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/comp_torque_rs.png}
    \caption{Evolución temporal del torque neto ($T_{\text{neto}}$) en [N.m]. Se contrasta el comportamiento con resistencia estatórica constante (RS Cte., línea naranja) y con resistencia estatórica variable (RS Variable, línea azul).}
    \label{fig:comparacion_torque_neto_RS}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/comp_velocidad_rs.png}
    \caption{Comportamiento de la velocidad angular mecánica ($\omega_m(t)$) en [rad/s] en función del tiempo. Se comparan las respuestas con resistencia estatórica constante (RS Cte., línea naranja) y con resistencia estatórica variable (RS Variable, línea azul).}
...
Finalmente, la Fig.~\ref{fig:pid_diagrama_522} muestra la implementación en Simulink de la ley de control~\eqref{eq:ley_pid_522}, con estas ganancias ya cargadas en sus bloques.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{imagenes/controlador_PID_diagrama.png}
    \caption{Implementación en Simulink del controlador PID externo de movimiento. La entrada es el error de velocidad $e_\omega=\omega_m^*-\omega_m$; su integral reconstruye el error de posición $e_{\theta_m}$ y una segunda integración provee la acción integral. Las tres ramas ponderadas por $b_a$, $K_{sa}$ y $K_{sia}$ se suman para entregar la consigna de torque $T_m^{*\prime}$, realizando la ley~\eqref{eq:ley_pid_522}.}
    \label{fig:pid_diagrama_522}
\end{figure}
...
\subsubsection{Diseño del observador}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.75\linewidth]{imagenes/pos-velocidad-simulacion_cont.png}
        \caption{Posición y velocidad de la articulación en función del tiempo siguiendo la consigna trapezoidal propuesta}
    \end{figure}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.75\linewidth]{imagenes/overshoot_transitorio_trap_pos.png}
        \caption{Overshoot del transitorio rampa-meseta de la posición de la articulación}
    \end{figure}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.75\linewidth]{imagenes/torque_pos_relacion.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{imagenes/pos_voltaje_comparacion.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{imagenes/perfil_trap_accel_comp.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{imagenes/comp_codcont_pos.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{imagenes/comp_codcont_vel.png}
