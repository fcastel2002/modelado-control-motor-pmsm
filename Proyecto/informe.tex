...
\begin{equation}
    T_m^{\ast}(t) = T_m^{\ast\prime}(t) + b_{eq}\omega_m(t) + \frac{k_l}{r}\sin{\Bigg( \frac{\theta_m(t)}{r}\Bigg)}
\end{equation}
...
i_{qs}^{r*}(t)= \frac{T_m^{\ast\prime}(t) + b_{eq}\omega_m(t) + \frac{k_l}{r}\sin{\Big( \frac{\theta_m(t)}{r}\Big)}}{\frac{3}{2}P_p\left[\lambda_m+(L_d-L_q)i_{ds}^{r}(t)\right]}
\end{gather*}
...
Dado que el objetivo del presente trabajo es controlar la posición y velocidad del eje, se propone la implementación de un controlador PID aplicando el método de sintonia serie, el objetivo es que este manipule directamente el torque aplicando consignas en el \textit{Modulador de Torque}, para ello se presenta la siguiente ley de control.
\begin{equation}
    T_m^{*\prime}(s)= b_a\cdot e_\omega(s) + K_{sa}\cdot e_{\theta_m}(s)+K_{sia}\cdot \frac{e_{\theta}(s)}{s}
\end{equation}
En la consigna se pide que el controlador tenga $\zeta = 0.75$ y $\omega_n = 800 \frac{rad}{s}$ para tener tales características, es necesario trabajar la ecuación que domina la dinámica del sistema reemplazando la ley de control para obtener la función de transferencia.
\begin{gather*}
...
\subsubsection{Diseño del observador}
    ...
    \begin{equation}
        ^2 + s K_{e_{\theta_m}}+K_{e\omega_m}=s^2-2p_{1,2}s+p_{1,2}^2
    \end{equation}
