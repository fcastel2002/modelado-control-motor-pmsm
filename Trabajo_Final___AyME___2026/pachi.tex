...
\begin{align}
    J_l\cdot \frac{d\omega_l(t)}{dt} = T_q(t) - b_l\cdot \omega_l(t) - T_l(t)  \\
...
Para evaluar el impacto dinámico, se analiza la migración de los polos del sistema LPV (los autovalores de la matriz $\mathbf{A}_o$). Se fija un punto de operación nominal de velocidad ($\omega_{m,o}$) y corriente de cuadratura ($I^r_{qs,o}$), y se realiza un barrido del parámetro $I^r_{ds,o}$ desde la zona de debilitamiento hasta la zona de reforzamiento de campo.

Sustituyendo estos valores en la matriz Jacobiana $\mathbf{A}_o$ analítica deducida previamente, se obtienen los autovalores para cada caso. Los resultados numéricos más relevantes se resumen en el Cuadro \ref{tab:migracion_polos}.
...
\subsubsection{Diseño del observador}
    ...
    0 \\ \frac{1}{J_{eq}}
    \end{bmatrix}u(t)+
    \bm{L}\big(y(t)-\bm{C}\bar{\bm{x}}(t)\big)\\
    \dot{\bar{\bm{x}}}(t)&=
    \big(\bm{A}-\bm{L}\bm{C}\big)\bar{\bm{x}}(t) +
    \begin{bmatrix}
    0 \\ \frac{1}{J_{eq}}
    \end{bmatrix}u(t)+\bm{L}\,y(t)
    \end{aligned}
    \end{gather*}
    Definiendo a $\bm{L}$ como la ganancia proporcional:
    \begin{equation*}
        \bm{L}=
        \begin{bmatrix}
            K_{e\theta_m}\\
    ...
    \begin{gather*}
    \begin{aligned}
        \det\!\left[s\bm{I}-\left(\bm{A}-\bm{L}\bm{C}\right)\right]&=0,\\
        \begin{bmatrix}
            s & 0 \\ 
            0 & s
        \end{bmatrix}-
