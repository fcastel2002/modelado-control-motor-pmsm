...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|l|c|c|}
        ...
        $\lambda'^r_m$ & Flujo magnético de imanes & 0.016 & Wb \\
        ...
        $T_{amb}^\circ$ & Temperatura ambiente (operación) & 25 & $^\circ\text{C}$ \\
        \hline
...
Se realizó un ensayo de excitación al escalón con $v_q = 5$ V y carga nula ($T_{ld}=0$), partiendo de condiciones iniciales de reposo y equilibrio térmico ($T_s^\circ(0)=20^\circ$C).
...
\left\{
    \begin{aligned}
        ...
        %
        \frac{d\omega_{mO}}{dt}(t) &= \frac{1}{J_{eq}} \left[ -b_{eq}\omega_{mO}(t) + \frac{3}{2}P_p \left[ \lambda'^r_m + (L_d - L_q)i_{dsO}^r(t) \right] i_{qsO}^r(t) - g \cdot k_l \sin\left(\frac{\theta_{mO}}{r}\right) - \frac{1}{r}T_{ld}(t) \right] = 0 \\[10pt]
        %
        \frac{di_{qsO}^r}{dt}(t) &= \frac{1}{L_q} \left[ v_{qsO}^r(t) - R_s(T_{sO}^\circ(t))i_{qsO}^r(t) - (\lambda'^r_m + L_d i_{dsO}^r(t))P_p \omega_{mO}(t) \right] = 0 \\[10pt]
        %
...
\left\{
    \begin{aligned}
        ...
        %
        \Delta \dot{\omega}_m(t) &= \frac{1}{J_{eq}} \left[ \frac{3}{2} P_p \left\{ [\lambda'^r_m + (L_d - L_q) \cdot i_{dsO}^r] \cdot \Delta i_{qs}^r(t) + (L_d - L_q) \cdot i_{qsO}^r \cdot \Delta i_{ds}^r(t) \right\} - b_{eq} \cdot \Delta \omega_m(t) \right. \\
        &\quad \left. - \frac{g \cdot k_l}{r^2} \cdot \cos\left(\frac{\theta_{mO}}{r}\right) \cdot \Delta \theta_m(t) - \frac{\Delta T_{ld}(t)}{r} \right] \\[10pt]
        %
        \Delta \dot{i}_{qs}^r(t) &= \frac{1}{L_q} \left[ \Delta v_{qs}^r(t) - R_{sO} \cdot \Delta i_{qs}^r(t) - R_{sREF} \alpha_{Cu} \Delta T_s^\circ(t) \cdot i_{qsO}^r - \{\lambda'^r_m + L_d \cdot i_{dsO}^r\} P_p \cdot \Delta \omega_m(t) \right. \\
        &\quad \left. - L_d P_p \omega_{mO} \cdot \Delta i_{ds}^r(t) \right] \\[10pt]
        %
        \Delta \dot{i}_{ds}^r(t) &= \frac{1}{L_d} \left[ \Delta v_{ds}^r(t) - R_{sO} \cdot \Delta i_{ds}^r(t) - R_{sREF} \alpha_{Cu} \Delta T_s^\circ(t) \cdot i_{dsO}^r + L_q \cdot i_{qsO}^r \cdot P_p \cdot \Delta \omega_m(t) \right. \\
        &\quad \left. + L_q \cdot P_p \cdot \omega_{mO} \cdot \Delta i_{qs}^r(t) \right] \\[10pt]
        %
        \Delta \dot{i}_{0s}^r(t) &= \frac{1}{L_{ls}} \left[ \Delta v_{0s}^r(t) - R_{sO} \cdot \Delta i_{0s}^r(t) - R_{sREF} \alpha_{Cu} \Delta T_s^\circ(t) \cdot i_{0s}^r \right] \\[10pt]
        %
        \Delta \dot{T}_s^\circ(t) &= \frac{1}{C_{ts}} \left[ \frac{3}{2} \cdot R_{sO} \cdot (2 \cdot i_{qsO}^r \cdot \Delta i_{qs}^r(t) + 2 \cdot i_{dsO}^r \cdot \Delta i_{ds}^r(t) + 4 \cdot i_{0sO}^r \cdot \Delta i_{0s}^r(t)) \right. \\
        &\quad + \frac{3 \alpha_{Cu} R_{sREF}}{2} (i_{qsO}^r(t)^2 + i_{dsO}^r(t)^2 + 2i_{0sO}^r(t)^2) \cdot \Delta T_s^\circ(t) \\
        &\quad \left. - \frac{1}{R_{ts-amb}} (\Delta T_s^\circ(t) - \Delta T_{amb}^\circ(t)) \right]
    \end{aligned}
\right.
\end{equation*}
...
A_0 = \begin{bmatrix}
0 & 1 & 0 & 0 & 0 & 0 \\
-\frac{g k_l \cos\left(\frac{\theta_{mO}}{r}\right)}{J_{eq} r^2} & -\frac{b_{eq}}{J_{eq}} & \frac{3 P_p (\lambda'^r_m + i_{dsO} (L_d - L_q))}{2 J_{eq}} & \frac{3 P_p i_{qsO} (L_d - L_q)}{2 J_{eq}} & 0 & 0 \\
0 & -\frac{P_p (\lambda'^r_m + L_d i_{dsO})}{L_q} & -\frac{R_s}{L_q} & -\frac{L_d P_p \omega_{mO}}{L_q} & 0 & -\frac{R_{sREF} \alpha_{Cu} i_{qsO}}{L_q} \\
0 & \frac{L_q P_p i_{qsO}}{L_d} & \frac{L_q P_p \omega_{mO}}{L_d} & -\frac{R_s}{L_d} & 0 & -\frac{R_{sREF} \alpha_{Cu} i_{dsO}}{L_d} \\
0 & 0 & 0 & 0 & -\frac{R_s}{L_{ls}} & -\frac{R_{sREF} \alpha_{Cu} i_{0sO}}{L_{ls}} \\
...
\begin{equation*}
\Gamma_{th} = -\frac{\frac{1}{R_{ts-amb}} - \frac{3 R_{sREF} \alpha_{Cu} (2 i_{0sO}^2 + i_{dsO}^2 + i_{qsO}^2)}{2}}{C_{ts}}
\end{equation*}
...
\[\dfrac{dT_s^\circ(t)}{dt} = \dfrac{1}{C_{ts}}\Big[\frac{3}{2}R_{s0}\cdot i_{qs}^{r\,2}(t)-\frac{1}{R_{ts-amb}}\big(T_s^\circ(t) - T_{amb}^\circ(t)\big)\Big]\]
Definiendo el vector de estados nominal $\mathbf{x}_{LTI} = [\theta_m, \omega_m, i_{qs}^r]^T$, la representación matricial de estado resulta:
...
\end{equation*}
Para representar el sistema completo, se incorpora esta EDO al modelo LTI, definiendo el vector de estado aumentado $\mathbf{x}_{aug} = [\theta_m, \omega_m, i_{qs}^r, i_{ds}^r]^T$. 
...
\begin{cases}
    ...
    \dfrac{dT_s^\circ(t)}{dt} = \dfrac{1}{C_{ts}}\Big[\frac{3}{2}R_{s0}\cdot \big(i_{qs}^{r\,2}(t)+i_{ds}^{r\,2}(t)\big)-\frac{1}{R_{ts-amb}}\big(T_s^\circ(t) - T_{amb}^\circ(t)\big)\Big]
\end{cases}
\end{equation}
\begingroup
\renewcommand{\arraystretch}{1.8} 
...
\begin{equation} \label{eq:torque_ids}
    T_m = \frac{3}{2} P_p \Big[ \lambda'^r_m + (L_d - L_q) I_{ds0}^r \Big] I_{qs0}^r
\end{equation}
...
\begin{equation}
    \mathbf{x}_{aug}(0) = \begin{bmatrix} \theta_m(0) \\ \omega_m(0) \\ i_{qs}^r(0) \\ i_{ds}^r(0) \end{bmatrix} = \begin{bmatrix} 0 \\ 0 \\ 0 \\ 0 \end{bmatrix}
\end{equation}
...
\begin{equation}
    \det(s\mathbf{I} - \mathbf{A}_{aug}) = \underbrace{\left(s + \frac{R_{s0}}{L_d}\right)}_{\text{Polo de } i_{ds}^r} \cdot \underbrace{\left[ s^3 + \left(\frac{b_{eq}}{J_{eq}} + \frac{R_{s0}}{L_q}\right)s^2 + \left(\frac{b_{eq} R_{s0}}{J_{eq} L_q} + \frac{3 P_p^2 {\lambda'^r_m}^2}{2 J_{eq} L_q}\right)s \right]}_{\Delta(s) \text{ --- Polinomio del subsistema nominal}}
\end{equation}
...
\boxed{
    G_1(s) = \frac{\theta_m(s)}{v_{qs}(s)} = \frac{\dfrac{3 P_p \lambda'^r_m}{2 J_{eq} L_q}}{s^3 + \left(\dfrac{b_{eq}}{J_{eq}} + \dfrac{R_{s0}}{L_q}\right)s^2 + \left(\dfrac{b_{eq} R_{s0}}{J_{eq} L_q} + \dfrac{3 P_p^2 {\lambda'^r_m}^2}{2 J_{eq} L_q}\right)s}
}
\end{equation}
...
\boxed{
    G_2(s) = \frac{\theta_m(s)}{T_l(s)} = \frac{-\dfrac{1}{r J_{eq}}\left(s + \dfrac{R_{s0}}{L_q}\right)}{s^3 + \left(\dfrac{b_{eq}}{J_{eq}} + \dfrac{R_{s0}}{L_q}\right)s^2 + \left(\dfrac{b_{eq} R_{s0}}{J_{eq} L_q} + \dfrac{3 P_p^2 {\lambda'^r_m}^2}{2 J_{eq} L_q}\right)s}
}
\end{equation}
...
\end{equation}
que solo se manifiesta si la condición inicial es no nula, decayendo exponencialmente con constante de tiempo $\tau_d = L_d / R_{s0}$. Esto confirma que el \textbf{modelo nominal de tercer orden} $\mathbf{x}_{LTI} = [\theta_m, \omega_m, i_{qs}^r]^T$ es suficiente para caracterizar completamente la relación entrada-salida del sistema.
...
La resistencia del estator varía con la temperatura según $R_s\big(T_s^\circ(t)\big)=R_{sREF}\big[1+\alpha_{Cu}\big(T_s^\circ(t)-T_{sREF}^\circ\big)\big]$ (Ec.~3.9 de la guía). Para evaluar su efecto sobre la dinámica del modelo LTI, se realiza un barrido alrededor del valor de operación $R_{s0}$, en el rango $[0{,}8\,R_{s0},\; 1{,}2\,R_{s0}]$.
...
\begin{itemize}
    ...
    \item $\mathbf{C}\mathbf{A}^2 = \begin{bmatrix} 0 & -\frac{b_{eq}}{J_{eq}} & \frac{3P_p\lambda'^r_m}{2J_{eq}} & 0 \end{bmatrix}$ --- combinación de $\omega_m$ e $i_{qs}^r$.
    \item $\mathbf{C}\mathbf{A}^3$: combinación que involucra $\omega_m$ e $i_{qs}^r$, pero \textbf{nunca} $i_{ds}^r$.
\end{itemize}
...
\begin{itemize}
    ...
    \item $\mathbf{C}_\omega\mathbf{A} = \begin{bmatrix} 0 & -\frac{b_{eq}}{J_{eq}} & \frac{3P_p\lambda'^r_m}{2J_{eq}} & 0 \end{bmatrix}$ --- combinación de $\omega_m$ e $i_{qs}^r$.
    \item $\mathbf{C}_\omega\mathbf{A}^2$: combinación de $\omega_m$ e $i_{qs}^r$.
    \item $\mathbf{C}_\omega\mathbf{A}^3$: combinación de $\omega_m$ e $i_{qs}^r$.
\end{itemize}
...
\begin{itemize}
    \item $\mathbf{B}_c = \begin{bmatrix} 0 \\ 0 \\ 1/L_q \\ 0 \end{bmatrix}$ --- la entrada actúa directamente sobre $i_{qs}^r$.
    \item $\mathbf{A}\mathbf{B}_c = \begin{bmatrix} 0 \\ \frac{3P_p\lambda'^r_m}{2J_{eq}L_q} \\ -\frac{R_{s0}}{L_q^2} \\ 0 \end{bmatrix}$ --- la acción se propaga a $\omega_m$ a través del torque.
    \item $\mathbf{A}^2\mathbf{B}_c$: la acción alcanza $\theta_m$ (vía la cadena $i_{qs}^r \to \omega_m \to \theta_m$).
...
La ley de control mínima impone internamente la tensión $v_{ds} = -L_q \, i_{qs}^r \, \omega_r$, proporcional al producto $i_{qs}^r \cdot \omega_m$. Esta tensión es no nula solo cuando ambas variables lo son simultáneamente, y su magnitud crece con el nivel de operación del sistema.
...
\begin{itemize}
    ...
    \item En el \textbf{modelo NL}, $i_{ds}^r(t)$ también decae exponencialmente con la misma constante de tiempo. Aunque existe un acoplamiento residual NL del tipo $\omega_r \cdot L_q \cdot i_{qs}^r$, la ley de control mínima ($v_{ds} = -L_q i_{qs}^r \omega_r$) lo cancela exactamente, y la ley complementaria ($v_{qs} = v_{qs}^* + \omega_r L_d i_{ds}^r$) elimina el efecto cruzado inverso.
...
\begin{equation}
    v_{ds}(t) = \underbrace{-L_q \, i_{qs}^r \, \omega_r}_{\text{ley mínima}} + v_{ds}^*(t), \quad
    v_{ds}^*(t) = \begin{cases}
    0 & t < 0{,}5 \text{ s} \\
...
\textbf{Análisis:}
\begin{itemize}
    \item En el \textbf{modelo LTI}, $v_{ds}^*$ afecta \textbf{únicamente} a $i_{ds}^r$, que crece exponencialmente hasta su valor estacionario $i_{ds,ss}^r = v_{ds}^*/R_{s0} = \pm 1{,}92$~A. Los demás estados ($\theta_m$, $\omega_m$, $i_{qs}^r$) permanecen \textbf{inalterados} por el desacoplamiento estructural de la matriz $A_{aug}$.
...
\begin{figure}[H]
    ...
    \caption{Evolución de la temperatura del bobinado \(T_s^\circ(t)\) bajo operación repetitiva, considerando \(T_{amb}^\circ=25^\circ\text{C}\), \(T_{Ld}=5\,\text{Nm}\) y la consigna trapezoidal de posición utilizada en el trabajo. Se observa que la temperatura supera el límite admisible \(T_{s,\max}=115^\circ\text{C}\) a los 225.9 s.}
